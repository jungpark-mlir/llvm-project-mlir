...
def main() -> None:
    ...
    sub1_output_node_name = "Sub1_Y"
    sub1_node = onnx.helper.make_node(
        name="Sub1",  # Name is optional.
        op_type="Sub",
        inputs=[const2_tensor_name, mul2_output_node_name],
        outputs=[sub1_output_node_name]
    )

    mul3_output_node_name = "Mul3_Y"
    mul3_node = onnx.helper.make_node(
        name="Mul3",  # Name is optional.
        op_type="Mul",
        inputs=[mul1_output_node_name, add0_output_node_name],
        outputs=[mul3_output_node_name]
    )
    ...
    add2_node = onnx.helper.make_node(
        ...
        op_type="Add",
        inputs=[sub1_output_node_name, mul3_output_node_name],
        outputs=[model_output_name]
    )

    # Create the graph (GraphProto)
    graph_def = onnx.helper.make_graph(
        nodes=[add0_node, rmean0_node, sub0_node, mul0_node, rmean1_node, add1_node, sqrt0_node, recip0_node, mul1_node, mul2_node, sub1_node, mul3_node, add2_node],
        name="norm",
    ...
    onnx.save(model_def, "normblock.onnx")
