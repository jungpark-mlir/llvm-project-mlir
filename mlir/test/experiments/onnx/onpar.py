...
def main() -> None:
    ...
    #X1 = onnx.helper.make_tensor_value_info(model_input_name1, onnx.TensorProto.FLOAT, [None, 256, 224, 224])
    X0 = onnx.helper.make_tensor_value_info(model_input_name0, onnx.TensorProto.FLOAT, [None, 768, 1, 256])
    X1 = onnx.helper.make_tensor_value_info(model_input_name1, onnx.TensorProto.FLOAT, [None, 768, 1, 256])
    X2 = onnx.helper.make_tensor_value_info(model_input_name2, onnx.TensorProto.FLOAT, [None, 768, 1, 256])
    X3 = onnx.helper.make_tensor_value_info(model_input_name3, onnx.TensorProto.FLOAT, [None, 768, 1, 256])
    ...
    conv1_node = onnx.helper.make_node(
        ...
        # https://github.com/onnx/onnx/blob/rel-1.9.0/docs/Operators.md#inputs-2---3
        inputs=[
            model_input_name1, conv1_W_initializer_tensor_name,
    ...
    conv2_node = onnx.helper.make_node(
        ...
        # https://github.com/onnx/onnx/blob/rel-1.9.0/docs/Operators.md#inputs-2---3
        inputs=[
            model_input_name2, conv2_W_initializer_tensor_name,
    ...
    # Dummy weights for conv.
    conv3_in_channels = 768
    conv3_out_channels = 768
    conv3_kernel_shape = (1, 1)
    conv3_pads = (0, 0, 0, 0)
    conv3_W = np.ones(shape=(conv3_out_channels, conv3_in_channels,
                             *conv3_kernel_shape)).astype(np.float32)
    conv3_B = np.ones(shape=(conv3_out_channels)).astype(np.float32)
    # Create the initializer tensor for the weights.
    conv3_W_initializer_tensor_name = "Conv3_W"
    conv3_W_initializer_tensor = create_initializer_tensor(
        name=conv3_W_initializer_tensor_name,
        tensor_array=conv3_W,
        data_type=onnx.TensorProto.FLOAT)
    ...
    conv3_node = onnx.helper.make_node(
        ...
        # https://github.com/onnx/onnx/blob/rel-1.9.0/docs/Operators.md#inputs-2---3
        inputs=[
            model_input_name3, conv3_W_initializer_tensor_name,
    ...
    graph_def = onnx.helper.make_graph(
        ...
        name="ConvNet",
        inputs=[X0, X1, X2, X3],  # Graph input
        ...
        initializer=[
            ...
            conv3_W_initializer_tensor, conv3_B_initializer_tensor
        ],
    )
    ...
    onnx.save(model_def, "convpar.onnx")
